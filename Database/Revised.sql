...
CREATE TABLE IF NOT EXISTS `basicdetails` (
  ...
  PRIMARY KEY (`USN`),
  UNIQUE KEY `Id` (`Id`),
  UNIQUE KEY `USN_2` (`USN`,`Mobile`,`Email`)
...
CREATE TABLE IF NOT EXISTS `updatedrive` (
  ...
  PRIMARY KEY (`USN`,`CompanyName`,`Date`),
  KEY `CompanyName` (`CompanyName`,`Date`),
  CONSTRAINT `updatedrive_ibfk_1` FOREIGN KEY (`USN`) REFERENCES `basicdetails` (`USN`) ON DELETE CASCADE,
  CONSTRAINT `updatedrive_ibfk_2` FOREIGN KEY (`CompanyName`, `Date`) REFERENCES `addpdrive` (`CompanyName`, `Date`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
('1cg12is011', 'Vishruth Harithsa', 'Apple', '2015-07-23', 1, 1, 1, 1, 1),
