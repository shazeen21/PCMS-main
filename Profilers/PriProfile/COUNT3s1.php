...
<html lang="en">
        ...
        <table class="table table-striped table-bordered templatemo-user-table">
          ...
          <tbody>
            ...
            if(isset($_POST['s1'])) { 
                ...
                $escapedSname = mysqli_real_escape_string($conn, $Sname);
                $searchParam = '%' . $escapedSname . '%';
                
                // Count query
                $countQuery = "SELECT COUNT(*) as total FROM basicdetails WHERE `Approve`='1' AND (`FirstName` LIKE '$searchParam' OR `LastName` LIKE '$searchParam')";
                ...
                // Main query
                $query = "SELECT * FROM basicdetails WHERE `Approve`='1' AND (`FirstName` LIKE '$searchParam' OR `LastName` LIKE '$searchParam')";
  ...
  <footer class="text-right">
    <p>Copyright &copy; 2001-2015 CIT-PMS | Developed by <a href="http://znumerique.azurewebsites.net" target="_parent">ZNumerique Technologies</a></p>
  </footer>         
  </div>
  </div>
  </div>    
