...
if(isset($_POST['s6'])) { 
    ...
    $stmt = mysqli_prepare($conn, "SELECT COUNT(*) FROM basicdetails WHERE `Approve`='1' AND `PU/Dip`>=?");
    mysqli_stmt_bind_param($stmt, "d", $Cpu);
    ...
    $stmt = mysqli_prepare($conn, "SELECT * FROM basicdetails WHERE `Approve`='1' AND `PU/Dip`>=?");
    mysqli_stmt_bind_param($stmt, "d", $Cpu);
    mysqli_stmt_execute($stmt);
